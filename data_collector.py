...
class DataCollector:
    ...
    def collect(self, url):
        ...
        if self.validate_select(soup.select(self.PRICE_ELEMENT)):
            ...
            data.append(['price', get_price(price_element)])
            ...
            for node in self.INFORMATION_TABLE_NODES:
                ...
                if self.validate_select(node_element):
                    ...
                    node_value = None
                data.append([node, node_value])

        return data

    def validate_select(self, select):
        return select.__len__() > 0 and select[0].string != self.VALUE_HIDDEN and select[0].string is not None


def get_price(price_element):
    price_string = int(price_element[0].string.replace(' ', '').replace('zł', ''))
